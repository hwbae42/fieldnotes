...
CLAUDE.md 처리 방식은 두 가지다:
- **Claude-specific 기능이 있을 때**: `CLAUDE.md`에 `@AGENTS.md` + 추가 지시만 작성.
- **Claude-specific 기능이 없을 때**: `CLAUDE.md`를 AGENTS.md 심링크로 처리 (`ln -s AGENTS.md CLAUDE.md`). 관리 파일이 하나로 줄어들고 두 도구 모두 동일한 내용을 읽는다.

본 fieldnotes는 Claude-specific 항목이 없으므로 심링크 방식을 택했다.
